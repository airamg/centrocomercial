...
public class TiendaDAOImpl implements ITiendaDAO {
	...
	public int create(Tienda tienda) {
		int codigo;
		final String SQL = "INSERT INTO tienda (nombre,descripcion,tipo,estado,horario_abrir,horario_cerrar) VALUES (?,?,?,?,?,?)";
		codigo = jdbctemplate.update(SQL, tienda.getNombre(),tienda.getDescripcion(), tienda.getTipo(), tienda.getEstado(),	tienda.getHorario_abrir(), tienda.getHorario_cerrar());
		return codigo;
	}
	...
	public Tienda getById(int codigo) {
		...
		final String SQL = "SELECT id,nombre,descripcion,tipo,estado,horario_abrir,horario_cerrar FROM tienda WHERE id=?";
		try {
	...
	public List<Tienda> getAll() {
		...
		final String SQL = "SELECT id,nombre,descripcion,tipo,estado,horario_abrir,horario_cerrar FROM tienda";
		try {
	...
	public int update(Tienda tienda) {
		int codigo = -1;
		final String SQL = "UPDATE tienda SET nombre=?,descripcion=?,tipo=?,estado=?,horario_abrir=?,horario_cerrar=? WHERE id=?";
		codigo = jdbctemplate.update(SQL, tienda.getNombre(),tienda.getDescripcion(), tienda.getTipo(), tienda.getEstado(),	tienda.getHorario_abrir(), tienda.getHorario_cerrar());
		return codigo;
	}
